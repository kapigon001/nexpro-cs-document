...
│                                                                  │
│   ハカる（測る・図る）                                           │
│   → 成果の定義と計測。何をもって健康とするか基準を決める         │
│                                                                  │
│   ミる（観る・診る）                                             │
│   → 利用状況と顧客の声の観察。変化や異変を見逃さない            │
│                                                                  │
│   カく（書く・描く）                                             │
│   → カルテ（記録）と処方箋（改善提案）。次のアクションを示す     │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
|----|------|---------|---------|
| 1 | テンプレート整備 | カく | 2月末 |
| 2 | オンボーディング動画制作（12本） | カく | 3月末 |
| 3 | ヘルススコア運用開始 | ハカる | 2月中旬 |
| 4 | Salesforce情報一元化 | カく | 3月末 |
| 5 | 担当制導入（30社/人上限） | ミる | 2/1開始済 |
...
```
ミッション：顧客の"かかりつけ医"として、成果の創出を軸に伴走支援する
│
├── ハカる（測る・図る）── 成果を定義し、計測する
│   ├── 1. 目標指標
│   ├── 2. ヘルススコア
│   └── 3. 顧客セグメンテーション
│
├── ミる（観る・診る）── 顧客の状態を観察し、診断する
│   ├── 4. 顧客ライフサイクル管理
│   ├── 5. 定期接点（定期検診）
│   ├── 6. 解約予兆検知（異変の早期発見）
│   └── 7. アップセル機会の発見
│
├── カく（書く・描く）── カルテと処方箋を記録・提示する
│   ├── 8. Salesforce情報一元化（カルテ）
│   ├── 9. テンプレート・動画整備（処方箋の標準化）
│   ├── 10. オンボーディング（初診・治療計画）
│   ├── 11. ナレッジ管理（症例集）
│   └── 12. メディアサイト（セルフケアガイド）
│
...
> 何をもって顧客が「健康」であるかの基準を定め、定量的に計測する
...
**割り振り考慮要素：** 業種（金融・医療は高工数）、規模、利用フェーズ、担当者の得意領域・経験

---

## ミる（観る・診る）

> 顧客の利用状況と声を継続的に観察し、変化や異変を見逃さない
...
|-----|------|
| 0-5分 | アイスブレイク |
| 5-15分 | 前四半期の成果振り返り（検査結果の共有） |
| 15-25分 | 課題ディスカッション（問診） |
| 25-40分 | 次四半期のゴール設定（治療計画） |
| 40-50分 | ネクプロからの提案（処方箋） |
| 50-60分 | 質疑応答・次回日程 |
...
> カルテ（記録）と処方箋（改善提案）を残し、次のアクションを示す
...
**事例蓄積フロー：**
```
成果発生 → 担当CSがSFに簡易記録 → 月次MTGで共有
→ 事例化判定 → 顧客に取材依頼 → 事例ドキュメント作成 → 横展開
```
...
|-------|----------|----------|------|
| 営業 | アップセル機会共有 | 顧客引継ぎ（ヒアリングシート） | 随時+月1合同MTG |
| 開発 | 顧客要望フィードバック | 新機能リリース情報 | 月1+随時 |
| サポート | - | 問い合わせ傾向、クレーム共有 | 週1 |
| マーケ | 事例取材可能顧客 | - | 随時 |
| 経営層 | KPIレポート | - | 月1 |
...
| Phase | 期間 | やること | 行動指針 |
|-------|------|---------|---------|
| **Phase 1: 基盤構築** | 1-2月 | 担当制開始、テンプレート整備、動画6本、ヘルススコア設計 | ハカる+カく |
| **Phase 2: 仕組み化** | 3月 | 酒井入社対応、動画12本完成、SF連携完了、検知自動化 | ミる+カく |
| **Phase 3: 拡充** | 4月〜 | 本格稼働、メディアサイト構築、コンサル型CS展開、NRR測定 | 全て |
