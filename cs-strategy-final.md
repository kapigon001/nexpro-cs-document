...
```
1 ミッション：顧客の"かかりつけ医"として、成果の創出を軸に伴走支援する
│
├── 3 行動指針
│   │
│   ├── ハカる
│   │   ├── 測る：目標指標・ヘルススコア
│   │   └── 図る：顧客セグメンテーション・ライフサイクル設計
│   │
│   ├── ミる
│   │   ├── 観る：定期接点・利用状況モニタリング
│   │   └── 診る：解約予兆検知・アップセル機会発見
│   │
│   └── カく
│       ├── 書く：Salesforce情報一元化・ナレッジ管理
│       └── 描く：テンプレート・動画・オンボーディング・メディアサイト
│
└── 支える基盤：組織体制 / 他部門連携 / リスク管理 / ロードマップ
```
...
│  │
│  ├─ 測る ── 数値で定量的に計測する
│  │  ├─ 1.1 結果指標（チャーン率、NRR、パイプライン転換率）
│  │  ├─ 1.2 先行指標（ヘルススコア、アクティブ率、接点頻度）
│  │  ├─ 1.3 活動指標（MTG数、QBR実施率、提案数）
│  │  └─ 1.4 ヘルススコア（6指標・100点満点）
│  │
│  └─ 図る ── 基準と計画を設計する
│     ├─ 2.1 顧客セグメンテーション（S/A/B/C）
│     ├─ 2.2 担当割り振り基準
│     └─ 2.3 顧客ライフサイクル設計（5フェーズ）
│
├─ ミる ── 顧客の状態を観察し、診断する
│  │
│  ├─ 観る ── 広く全体の状況を観察する
│  │  ├─ 3.1 利用状況モニタリング（SF自動インポート）
│  │  ├─ 3.2 定期接点（QBR＝定期検診）
│  │  └─ 3.3 定期メール配信
│  │
│  └─ 診る ── 個別に深く診断する
